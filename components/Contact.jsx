...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://grover-portfolio.onrender.com/send-email", {
        method: "POST",
        headers: {
      ...
      >
        <label className="flex flex-col">
          ...
          <input
            ...
            className="bg-bgPrimaryDark py-4 px-6 placeholder:text-ctnSecondaryDark rounded-lg outline-none border-none font-medium text-ctnPrimaryDark placeholder:text-sm md:placeholder:text-lg"
        ...
        <button
          type="submit"
          disabled={loading}
          className="bg-primary py-3 px-8 rounded-xl outline-none w-fit text-white font-bold shadow-md shadow-primary hover:scale-105 transition-transform duration-300"
        >
